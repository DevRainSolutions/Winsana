...
namespace DevRain.Asana.API.Services.API
{
    public class LoadDataService
    {
        ...
        public async Task LoadWorkspaceTags(long id, bool firstLoad = true)
        {
            if (firstLoad)
                AddOperation();
            var tags = await new AsanaApiRepository().GetTags(id, new List<string> {"id", "name"});
            ;
            ;
        ...
        {
            var workspaces = await new AsanaApiRepository().GetWorkspaces();
        ...
        {
            var users = await new AsanaApiRepository().GetUsers(new List<string>() {"workspaces"});
